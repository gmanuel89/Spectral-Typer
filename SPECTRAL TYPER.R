...
functions_mass_spectrometry <- function() {
    ...
    ################## FUNCTIONS - MASS SPECTROMETRY 2017.06.22 ################
    ...
    # The outcome list must be: b (for benign) - m (for malignant) - o (for other)
    ...
    remove_low_intensity_peaks <<- function(peaks, low_intensity_peak_removal_threshold_percent = 1, low_intensity_peak_removal_threshold_method = "element-wise", allow_parallelization = FALSE) {
        ...
        if (low_intensity_peak_removal_threshold_percent > 0 && low_intensity_peak_removal_threshold_percent < 100) {
            ...
            if (low_intensity_peak_removal_threshold_method == "element-wise" || low_intensity_peak_removal_threshold_method == "spectrum-wise") {
                ...
                if (isMassPeaksList(peaks)) {
                    ...
                    if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                        ...
                        # Run the function
                        peaks_filtered <- list()
                        peaks_filtered <- foreach(i = 1:length(peaks), .packages = "MALDIquant") %dopar% {
                            peaks_filtered[[i]] <- intensity_filtering_subfunction_element(peaks[[i]], low_intensity_peak_removal_threshold_percent = low_intensity_peak_removal_threshold_percent)
                        }
                        names(peaks_filtered) <- list_names
                    } else {
                        ### SINGLE CORE
                        peaks_filtered <- lapply(peaks, FUN = function(peaks) intensity_filtering_subfunction_element(peaks, low_intensity_peak_removal_threshold_percent))
                    }
            ...
            if (low_intensity_peak_removal_threshold_method == "whole") {
                ...
                if (isMassPeaksList(peaks)) {
                    ...
                    if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                        ...
                        peaks_filtered <- list()
                        peaks_filtered <- foreach(i = 1:length(peaks), .packages = "MALDIquant") %dopar% {
                            peaks_filtered[[i]] <- intensity_filtering_subfunction_whole(peaks[[i]], low_intensity_peak_removal_threshold_percent = low_intensity_peak_removal_threshold_percent, highest_intensity = highest_intensity)
                        }
                        names(peaks_filtered) <- list_names
                    } else {
                        ### SINGLE CORE
            ...
            names(peaks_filtered) <- peaks_names
    ...
    replace_backslash <<- function(spectra, allow_parallelization = FALSE) {
        ...
        if (isMassSpectrumList(spectra) || isMassPeaksList(spectra)) {
            ...
            if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                ...
                spectra <- foreach(i = 1:length(spectra), .packages = "MALDIquant") %dopar% {
                    spectra[[i]] <- backslash_replacing_subfunction(spectra[[i]])
                }
                names(spectra) <- list_names
            } else {
                spectra <- lapply(spectra, FUN = function(spectra) backslash_replacing_subfunction(spectra))
            }
    ...
    import_spectra <<- function(filepath, spectra_format = "imzML", mass_range = NULL, allow_parallelization = FALSE, spectral_names = "name", replace_sample_name_field = TRUE, remove_empty_spectra = TRUE) {
        ...
        spectra <- NULL
        ### Try to import the spectra from files... If any task fails, it will be skipped since it is in the try() function, so the spectra remain NULL
        try({
            ### imzML
            ...
            ### Return NULL if all the spectra are gone
            if (length(spectra) == 0) {
                spectra <- NULL
            }
        }, silent = TRUE)
        ### Return the variable spectra
        return(spectra)
    }
    ...
    replace_sample_name <<- function(spectra, spectra_format = "imzML", allow_parallelization = FALSE) {
        ...
        if (isMassSpectrumList(spectra) || isMassPeaksList(spectra)) {
            ...
            if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                ...
                spectra <- foreach(i = 1:length(spectra), .packages = "MALDIquant") %dopar% {
                    spectra[[i]] <- name_replacing_subfunction(spectra[[i]], spectra_format = spectra_format)
                }
                names(spectra) <- list_names
            } else {
                spectra <- lapply(spectra, FUN = function(spectra) name_replacing_subfunction(spectra, spectra_format = spectra_format))
        ...
        if (is.null(names(spectra))) {
            ...
            if (type == "name") {
                ...
                if (replace_sample_name_field == TRUE) {
                    ...
                    spectra_temp <- replace_sample_name(spectra, spectra_format = spectra_format, allow_parallelization = allow_parallelization)
                    ##### More spectra
                    if (isMassSpectrumList(spectra) || isMassPeaksList(spectra)) {
                        # Generate a vector with all the spectra names
                        spectra_names_vector <- character()
        ...
        if (isMassSpectrumList(spectra)) {
            ...
            for (f in 1:number_of_folds) {
                ...
                if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                    ...
                    # Preserve the list names
                    if (!is.null(names(spectra_temp))) {
                        list_names <- names(spectra_temp)
                    } else {
                        list_names <- NULL
                    }
    ...
    average_spectrum_bars <<- function(spectra, SNR = 5, peak_picking_algorithm = "SuperSmoother", tof_mode = "linear", tolerance_ppm = NULL, mass_range_plot = c(4000,15000), graph_title = "Spectrum", average_spectrum_color = "black", peak_points = "yes", points_color = "red", bar_width = 40, bar_color = "blue") {
        ...
        peaks_all <- align_and_filter_peaks(peaks_all, tof_mode = tof_mode, peak_filtering_frequency_threshold_percent = 5, low_intensity_peak_removal_threshold_percent = 0, tolerance_ppm = tolerance_ppm)
        # Empty the lists
        peaks_average <- list()
        peaks <- list()
    ...
    peak_picking <<- function(spectra, peak_picking_algorithm = "SuperSmoother", tof_mode = "linear", SNR = 3, allow_parallelization = FALSE, deisotope_peaklist = FALSE, envelope_peaklist = FALSE) {
        ...
        if (isMassSpectrum(spectra)) {
            ...
            if (((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply"))) {
                ...
                peaks <- list()
                peaks <- foreach(i = 1:length(spectra), .packages = "MALDIquant") %dopar% {
                    peaks[[i]] <- detectPeaks(spectra[[i]], method = peak_picking_algorithm, halfWindowSize = half_window_size, SNR = SNR)
                }
                names(peaks) <- list_names
            } else {
                peaks <- detectPeaks(spectra, method = peak_picking_algorithm, halfWindowSize = half_window_size, SNR = SNR)
                names(peaks) <- names(spectra)
            }
    ...
    deisotope_peaks <<- function(peaks, pattern_model_correlation = 0.95, isotopic_tolerance = 10^(-4), isotope_pattern_distance = 1.00235, isotopic_pattern_size = 3L:10L, allow_parallelization = FALSE) {
        ...
        if (isMassPeaksList(peaks)) {
            ...
            if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                ...
                # Run the algorithm
                peaks_deisotoped <- list()
                peaks_deisotoped <- foreach(i = 1:length(peaks), .packages = "MALDIquant") %dopar% {
                    peaks_deisotoped[[i]] <- monoisotopicPeaks(peaks[[i]], minCor = pattern_model_correlation, tolerance = isotopic_tolerance, distance = isotope_pattern_distance, size = isotopic_pattern_size)
                }
                names(peaks_deisotoped) <- list_names
            } else {
                # Run the algorithm
                peaks_deisotoped <- monoisotopicPeaks(peaks, minCor = pattern_model_correlation, tolerance = isotopic_tolerance, distance = isotope_pattern_distance, size = isotopic_pattern_size)
            }
    ...
    envelope_peaks <<- function(peaks, allow_parallelization = FALSE) {
        ...
        if (isMassPeaksList(peaks)) {
            ...
            if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                ...
                # Run the algorithm
                peaks_enveloped <- list()
                peaks_enveloped <- foreach(i = 1:length(peaks), .packages = "MALDIquant") %dopar% {
                    peaks_enveloped[[i]] <- envelope_peaklist_subfunction(peaks[[i]])
                }
                names(peak_enveloped) <- list_names
            } else {
                # Run the algorithm
                peaks_enveloped <- lapply(peaks, FUN = function(peaks) envelope_peaklist_subfunction(peaks))
            }
                ...
                try({
                    ...
                    if (isMassPeaksList(peaks_aligned)) {
                        if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                            ...
                            # Preserve the list names
                            if (!is.null(names(peaks_aligned))) {
                                list_names <- names(peaks_aligned)
                            } else {
                                list_names <- NULL
                            }
                            peaks_aligned <- foreach(i = 1:length(peaks_aligned), .packages = "MALDIquant") %dopar% {
                                peaks_aligned[[i]] <- warpMassPeaks(peaks_aligned[[i]], w = warping_functions)
                            }
                            names(peaks_aligned) <- list_names
                        } else {
    ...
    single_model_classification_of_spectra <<- function(spectra, model_x, model_name = "model", preprocessing_parameters = NULL, peak_picking_algorithm = "SuperSmoother", deisotope_peaklist = FALSE, peak_picking_SNR = 5, peak_filtering_frequency_threshold_percent = 5, low_intensity_peak_removal_threshold_percent = 1, low_intensity_peak_removal_threshold_method = "element-wise", tof_mode = "linear", allow_parallelization = FALSE, pixel_grouping = c("single", "hca", "moving window average", "graph"), number_of_hca_nodes = 5, moving_window_size = 5, seed = 12345, correlation_method_for_adjacency_matrix = "pearson", correlation_threshold_for_adjacency_matrix = 0.95, pvalue_threshold_for_adjacency_matrix = 0.05, max_GA_generations = 10, iterations_with_no_change = 5, number_of_spectra_partitions = 1, partitioning_method = "space", classification_mode_graph = c("average spectra", "single spectra clique"), plot_figures = TRUE, plot_graphs = TRUE, plot_legends = c("sample name", "legend", "plot name"), features_to_use_for_graph = c("all", "model"), tolerance_ppm = NULL) {
        ...
        if (isMassSpectrumList(spectra)) {
            ...
            if (pixel_grouping == "single" || number_of_hca_nodes == 1 || moving_window_size >= length(spectra)) {
                ...
                peaks <- align_and_filter_peaks(peaks, tof_mode = tof_mode, peak_filtering_frequency_threshold_percent = peak_filtering_frequency_threshold_percent, tolerance_ppm = tolerance_ppm)
            ...
            ### Generate the intensity matrix with the features from the model
            final_sample_matrix <- generate_custom_intensity_matrix(spectra, custom_feature_vector = features_model, tof_mode = tof_mode, preprocessing_parameters = NULL, peak_picking_algorithm = peak_picking_algorithm, peak_picking_SNR = 3, peak_filtering_frequency_threshold_percent = NULL, low_intensity_peak_removal_threshold_percent = 0, low_intensity_peak_removal_threshold_method = "element-wise", allow_parallelization = allow_parallelization, deisotope_peaklist = deisotope_peaklist, tolerance_ppm = tolerance_ppm)
    ...
    feature_selection <<- function(training_set, feature_selection_method = "ANOVA", features_to_select = 20, selection_method = "pls", selection_metric = "Kappa", correlation_method = "pearson", correlation_threshold = 0.75, auc_threshold = 0.7, cv_repeats_control = 5, k_fold_cv_control = 10, discriminant_attribute = "Class", non_features = c("Sample", "Class", "THY"), seed = NULL, automatically_select_features = FALSE, generate_plots = TRUE, preprocessing = c("center","scale"), allow_parallelization = FALSE, feature_reranking = FALSE) {
        ...
        if (feature_selection_method == "ANOVA") {
            feature_list <- names(training_set[,!(names(training_set) %in% non_features)])
            ...
            for (feature in feature_list) {
                ...
                intensity_vector <- training_set[, feature]
                # Compute an ANOVA based upon the discriminant attribute
                feature_ANOVA <- aov(intensity_vector ~ training_set[, discriminant_attribute])
                # Extract the p-value
        ...
        if (feature_selection_method == "kruskal" || feature_selection_method == "kruskal-wallis" || feature_selection_method == "Kruskal-Wallis") {
            ...
            for (feature in feature_list){
                ...
                intensity_vector <- training_set [,feature]
                # Compute an ANOVA based upon the discriminant attribute
                feature_kruskal <- kruskal.test(intensity_vector ~ training_set[,discriminant_attribute])
                # Extract the p-value
        ...
        if (feature_selection_method == "correlation") {
            ...
            training_set_features <- training_set[,!(names(training_set) %in% non_features)]
            # Compute the correlation
            feature_correlation <- cor(training_set_features, method = correlation_method)
        ...
        if (feature_selection_method == "ROC" || feature_selection_method == "roc") {
            ...
            if (automatically_select_features == TRUE) {
                ...
                for (feature in feature_list){
                    ...
                    feature_ROC <- roc(response = training_set[,discriminant_attribute], predictor = training_set[,feature])
                    # Extract the AUC
                ...
                for (f in 1:length(feature_list)) {
                    # Compute the ROC of the feature
                    feature_ROC <- roc(response = training_set[,discriminant_attribute], predictor = training_set[,feature_list[f]])
        ...
        # Return the values
        return(list(training_set_feature_selection = training_set_feature_selection, predictors_feature_selection = predictors_feature_selection, feature_selection_graphics = feature_selection_graphics, feature_weights = feature_weights, variable_importance = variable_importance, fs_model_performance = fs_model_performance, fs_model = fs_model))
    }
    ...
    wrapper_rfe <<- function(peaklist, features_to_select = 20, selection_method = "pls", selection_metric = "Accuracy", cv_repeats_control = 5, k_fold_cv_control = 10, discriminant_attribute = "Class", non_features = c("Sample", "Class"), seed = NULL, automatically_select_features = TRUE, generate_plots = TRUE, preprocessing = c("center","scale"), allow_parallelization = FALSE, feature_reranking = TRUE, test_set = NULL, positive_class_cv = "HP") {
        ...
        ### External validation
        if (!is.null(test_set) && (is.matrix(test_set) || is.data.frame(test_set))) {
            ...
            test_set <- test_set[, names(peaklist_feature_selection)]
            predicted_classes_model <- predict(fs_model, newdata = test_set[,!(names(test_set) %in% non_features)])
            # Create the outcomes dataframe
            classification_results_model <- data.frame(Sample = test_set$Sample, Predicted = predicted_classes_model, True = test_set$Class)
            ...
            model_performance_confusion_matrix <- confusionMatrix(data = predicted_classes_model, reference = test_set$Class, positive = positive_class_cv, mode = "everything", dnn = c("Predicted", "Actual"))
            ## ROC analysis
            model_roc <- list()
    ...
    embedded_rfe <<- function(training_set, features_to_select = 20, selection_method = "pls", model_tuning = c("embedded", "after", "none"), model_tune_grid = list(ncomp = 1:5), selection_metric = "Accuracy", cv_repeats_control = 5, k_fold_cv_control = 10, discriminant_attribute = "Class", non_features = c("Sample", "Class"), seed = NULL, automatically_select_features = TRUE, generate_plots = TRUE, preprocessing = c("center","scale"), allow_parallelization = FALSE, feature_reranking = TRUE, test_set = NULL, positive_class_cv = NULL) {
        ...
        if (automatically_select_features == TRUE) {
            ...
            if (is.na(all(predictors_feature_selection))) {
                predictors_feature_selection <- predictors(rfe_model)
            }
        } else {
            predictors_feature_selection <- predictors(rfe_model$fit)[1:features_to_select]
            if (length(predictors_feature_selection[is.na(predictors_feature_selection)]) > 0) {
                predictors_feature_selection <- predictors(rfe_model)[1:features_to_select]
            }
        }
        ##### Plots
        if (generate_plots == TRUE) {
            ...
            try({
                # Model tuning plot will fail if no tuning has been performed
        ...
        if ((!is.null(model_tuning) && model_tuning == "after") && (!is.null(model_tune_grid) || (is.list(model_tune_grid) && length(model_tune_grid) > 0))) {
            ...
            if (generate_plots == TRUE) {
                model_tuning_graphics <- NULL
                try({
                    model_tuning_graphics <- plot(fs_model_tuning, type = c("g","o"))
                }, silent = TRUE)
            } else {
                model_tuning_graphics <- NULL
            }
    ...
    model_ensemble_embedded_fs <<- function(training_set, features_to_select = 20, common_features_to_select = 0, model_tuning = "after", selection_metric = "Accuracy", discriminant_attribute = "Class", non_features = c("Sample", "Class"), seed = 12345, automatically_select_features = FALSE, generate_plots = TRUE, cv_repeats_control = 5, k_fold_cv_control = 3, preprocessing = c("center", "scale"), allow_parallelization = TRUE, feature_reranking = FALSE, try_combination_of_parameters = FALSE, outcome_list = c("benign", "malignant"), progress_bar = NULL) {
        ...
        pls_model_cv_performance_parameter_list <- pls_model_rfe$model_cv_performance_parameter_list
        ...
        svmRadial_model_cv_performance_parameter_list <- svmRadial_model_rfe$model_cv_performance_parameter_list
        ...
        svmPoly_model_cv_performance_parameter_list <- svmPoly_model_rfe$model_cv_performance_parameter_list
        ...
        svmLinear_model_ID <- "svm"
        svmLinear_model_performance <- svmLinear_model_rfe$fs_model_performance
        svmLinear_model_cross_validation_confusion_matrix <- svmLinear_model_rfe$cross_validation_confusion_matrix
        svmLinear_model_cross_validation_confusion_matrix_df <- svmLinear_model_rfe$cross_validation_confusion_matrix_df
        svmLinear_model_cv_performance_parameter_list <- svmLinear_model_rfe$model_cv_performance_parameter_list
        ...
        rf_model_cv_performance_parameter_list <- rf_model_rfe$model_cv_performance_parameter_list
        ...
        nbc_model_cv_performance_parameter_list <- nbc_model_rfe$model_cv_performance_parameter_list
        ...
        knn_model_cv_performance_parameter_list <- knn_model_rfe$model_cv_performance_parameter_list
        ...
        nnet_model_cv_performance_parameter_list <- nnet_model_rfe$model_cv_performance_parameter_list
        ...
        # Partial Least Squares
        PLS_model_list <- list(model = pls_model, class_list = pls_model_class_list, outcome_list = outcome_list, features_model = pls_model_features, model_ID = pls_model_ID, model_performance = pls_model_performance, cross_validation_confusion_matrix = pls_model_cross_validation_confusion_matrix, cross_validation_confusion_matrix_df = pls_model_cross_validation_confusion_matrix_df, model_cv_performance_parameter_list = pls_model_cv_performance_parameter_list, external_validation_confusion_matrix = pls_model_external_validation_confusion_matrix, external_validation_confusion_matrix_df = pls_model_external_validation_confusion_matrix_df, model_external_performance_parameter_list = pls_model_external_performance_parameter_list)
        # Support Vector Machine (with Radial Basis Kernel function)
        RSVM_model_list <- list(model = svmRadial_model, class_list = svmRadial_model_class_list, outcome_list = outcome_list, features_model = svmRadial_model_features, model_ID = svmRadial_model_ID, model_performance = svmRadial_model_performance, cross_validation_confusion_matrix = svmRadial_model_cross_validation_confusion_matrix, cross_validation_confusion_matrix_df = svmRadial_model_cross_validation_confusion_matrix_df, model_cv_performance_parameter_list = svmRadial_model_cv_performance_parameter_list, external_validation_confusion_matrix = svmRadial_model_external_validation_confusion_matrix, external_validation_confusion_matrix_df = svmRadial_model_external_validation_confusion_matrix_df, model_external_performance_parameter_list = svmRadial_model_external_performance_parameter_list)
        # Support Vector Machine (with Polynomial Kernel function)
        PSVM_model_list <- list(model = svmPoly_model, class_list = svmPoly_model_class_list, outcome_list = outcome_list, features_model = svmPoly_model_features, model_ID = svmPoly_model_ID, model_performance = svmPoly_model_performance, cross_validation_confusion_matrix = svmPoly_model_cross_validation_confusion_matrix, cross_validation_confusion_matrix_df = svmPoly_model_cross_validation_confusion_matrix_df, model_cv_performance_parameter_list = svmPoly_model_cv_performance_parameter_list, external_validation_confusion_matrix = svmPoly_model_external_validation_confusion_matrix, external_validation_confusion_matrix_df = svmPoly_model_external_validation_confusion_matrix_df, model_external_performance_parameter_list = svmPoly_model_external_performance_parameter_list)
        # Support Vector Machine (with Linear Kernel function)
        LSVM_model_list <- list(model = svmLinear_model, class_list = svmLinear_model_class_list, outcome_list = outcome_list, features_model = svmLinear_model_features, model_ID = svmLinear_model_ID, model_performance = svmLinear_model_performance, cross_validation_confusion_matrix = svmLinear_model_cross_validation_confusion_matrix, cross_validation_confusion_matrix_df = svmLinear_model_cross_validation_confusion_matrix_df, model_cv_performance_parameter_list = svmLinear_model_cv_performance_parameter_list, external_validation_confusion_matrix = svmLinear_model_external_validation_confusion_matrix, external_validation_confusion_matrix_df = svmLinear_model_external_validation_confusion_matrix_df, model_external_performance_parameter_list = svmLinear_model_external_performance_parameter_list)
        # Random Forest
        RF_model_list <- list(model = rf_model, class_list = rf_model_class_list, outcome_list = outcome_list, features_model = rf_model_features, model_ID = rf_model_ID, model_performance = rf_model_performance, cross_validation_confusion_matrix = rf_model_cross_validation_confusion_matrix, cross_validation_confusion_matrix_df = rf_model_cross_validation_confusion_matrix_df, model_cv_performance_parameter_list = rf_model_cv_performance_parameter_list, external_validation_confusion_matrix = rf_model_external_validation_confusion_matrix, external_validation_confusion_matrix_df = rf_model_external_validation_confusion_matrix_df, model_external_performance_parameter_list = rf_model_external_performance_parameter_list)
        # Naive Bayes Classifier
        NBC_model_list <- list(model = nbc_model, class_list = nbc_model_class_list, outcome_list = outcome_list, features_model = nbc_model_features, model_ID = nbc_model_ID, model_performance = nbc_model_performance, cross_validation_confusion_matrix = nbc_model_cross_validation_confusion_matrix, cross_validation_confusion_matrix_df = nbc_model_cross_validation_confusion_matrix_df, model_cv_performance_parameter_list = nbc_model_cv_performance_parameter_list, external_validation_confusion_matrix = nbc_model_external_validation_confusion_matrix, external_validation_confusion_matrix_df = nbc_model_external_validation_confusion_matrix_df, model_external_performance_parameter_list = nbc_model_external_performance_parameter_list)
        # K-Nearest Neighbor
        ...
        if (length(common_features_list) > 0) {
            ...
            training_set_common_features <- training_set[, c(common_features_list, non_features)]
        } else {
    ...
    single_model_training_and_tuning <<- function(training_set_feature_selection, non_features = c("Sample", "Class"), discriminant_attribute = "Class", preprocessing = c("center", "scale"), selection_method = "pls", model_tune_grid = list(), selection_metric = "Accuracy", cv_repeats_control = 5, k_fold_cv_control = 10, seed = NULL, generate_plots = TRUE, test_set = NULL, positive_class_cv = "HP", allow_parallelization = TRUE) {
        ...
        if (!is.null(test_set) && (is.matrix(test_set) || is.data.frame(test_set))) {
            ...
            classification_results_model <- data.frame(Sample = test_set$Sample, Predicted = predicted_classes_model, True = test_set$Class)
            # Generate the confusion matrix to evaluate the performances (take the first class as positive if not specified)
            if (is.null(positive_class_cv) || !(positive_class_cv %in% levels(as.factor(training_set_feature_selection[, discriminant_attribute])))) {
                positive_class_cv <- levels(as.factor(training_set_feature_selection[, discriminant_attribute]))[1]
            }
            model_performance_confusion_matrix <- confusionMatrix(data = predicted_classes_model, reference = test_set$Class, positive = positive_class_cv, , dnn = c("Predicted", "Actual"), mode = "everything")
            ## ROC analysis
            model_roc <- list()
            roc_curve <- roc(response = as.numeric(classification_results_model$True), predictor = as.numeric(classification_results_model$Predicted))
            model_roc[[1]] <- roc_curve$auc
        ...
        ### Return
        return(list(fs_model = fs_model, fs_model_performance = fs_model_performance, class_list = levels(as.factor(training_set_feature_selection[, discriminant_attribute])), feature_list = colnames(training_set_feature_selection[, !(names(training_set_feature_selection) %in% non_features)]), model_performance_confusion_matrix = model_performance_confusion_matrix, pie_chart_classification = pie_chart_classification, model_roc = model_roc))
    }
    ...
    automated_single_model_training_and_tuning <<- function(training_set_feature_selection, non_features = c("Sample", "Class"), discriminant_attribute = "Class", preprocessing = c("center", "scale"), selection_method = "pls", model_tune_grid = list(), selection_metric = "Accuracy", cv_repeats_control = 5, k_fold_cv_control = 10, seed = NULL, generate_plots = TRUE, test_set = NULL, positive_class_cv = "HP", allow_parallelization = TRUE, try_combination_of_parameters = TRUE) {
        ...
        if (try_combination_of_parameters == TRUE) {
            ...
            # Run every combination, storing the result if good
            for (comb in 1:length(parameter_combination)) {
                single_model <- single_model_training_and_tuning(training_set_feature_selection, non_features = non_features, discriminant_attribute = discriminant_attribute, preprocessing = parameter_combination[[comb]]$preprocessing, selection_method = selection_method, model_tune_grid = model_tune_grid, selection_metric = selection_metric, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, seed = seed, generate_plots = generate_plots, test_set = test_set, positive_class_cv = positive_class_cv, allow_parallelization = allow_parallelization)
                ### Check (and store) the performance values and the model
            ...
            ### NO combinations, run the single function...
            best_fs_model <- single_model_training_and_tuning(training_set_feature_selection, non_features = non_features, discriminant_attribute = discriminant_attribute, preprocessing = preprocessing, selection_method = selection_method, model_tune_grid = model_tune_grid, selection_metric = selection_metric, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, seed = seed, generate_plots = generate_plots, test_set = test_set, positive_class_cv = positive_class_cv, allow_parallelization = allow_parallelization)
        }
        ### Return the values
    ...
    model_ensemble_training_and_tuning <<- function(training_set_feature_selection, non_features = c("Sample", "Class"), discriminant_attribute = "Class", preprocessing = c("center", "scale"), selection_metric = "Accuracy", cv_repeats_control = 5, k_fold_cv_control = 10, seed = NULL, generate_plots = TRUE, test_set = NULL, positive_class_cv = "HP", allow_parallelization = TRUE, try_combination_of_parameters = TRUE, outcome_list = c("benign", "malignant"), progress_bar = NULL) {
        ...
        # Partial Least Squares
        pls_model_training_and_tuning <- automated_single_model_training_and_tuning(training_set_feature_selection, non_features = non_features, discriminant_attribute = discriminant_attribute, preprocessing = preprocessing, selection_method = "pls", model_tune_grid = list(ncomp = 1:5), selection_metric = selection_metric, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, seed = seed, generate_plots = generate_plots, test_set = test_set, positive_class_cv = positive_class_cv, allow_parallelization = allow_parallelization, try_combination_of_parameters = try_combination_of_parameters)
        ...
        # Support Vector Machine (with Polynomial Kernel function)
        svmPoly_model_training_and_tuning <- automated_single_model_training_and_tuning(training_set_feature_selection, non_features = non_features, discriminant_attribute = discriminant_attribute, preprocessing = preprocessing, selection_method = "svmPoly", model_tune_grid = list(C = 10^(-5:5), degree = 1:5, scale = 1), selection_metric = selection_metric, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, seed = seed, generate_plots = generate_plots, test_set = test_set, positive_class_cv = positive_class_cv, allow_parallelization = allow_parallelization, try_combination_of_parameters = try_combination_of_parameters)
        ...
        # Random Forest
        rf_model_training_and_tuning <- automated_single_model_training_and_tuning(training_set_feature_selection, non_features = non_features, discriminant_attribute = discriminant_attribute, preprocessing = preprocessing, selection_method = "rf", model_tune_grid = list(mtry = seq(1,5, by = 1)), selection_metric = selection_metric, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, seed = seed, generate_plots = generate_plots, test_set = test_set, positive_class_cv = positive_class_cv, allow_parallelization = allow_parallelization, try_combination_of_parameters = try_combination_of_parameters)
        ...
        # Naive Bayes Classifier
        nbc_model_training_and_tuning <- automated_single_model_training_and_tuning(training_set_feature_selection, non_features = non_features, discriminant_attribute = discriminant_attribute, preprocessing = preprocessing, selection_method = "nb", model_tune_grid = list(fL = seq(0, 1, by = 0.2), usekernel = c(TRUE, FALSE), adjust = c(TRUE, FALSE)), selection_metric = selection_metric, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, seed = seed, generate_plots = generate_plots, test_set = test_set, positive_class_cv = positive_class_cv, allow_parallelization = allow_parallelization, try_combination_of_parameters = try_combination_of_parameters)
        ...
        # Neural Network
        nnet_model_training_and_tuning <- automated_single_model_training_and_tuning(training_set_feature_selection, non_features = non_features, discriminant_attribute = discriminant_attribute, preprocessing = preprocessing, selection_method = "nnet", model_tune_grid = list(size = seq(1,5, by = 1), decay = seq(0,2,by = 1)), selection_metric = selection_metric, cv_repeats_control = cv_repeats_control, k_fold_cv_control = k_fold_cv_control, seed = seed, generate_plots = generate_plots, test_set = test_set, positive_class_cv = positive_class_cv, allow_parallelization = allow_parallelization, try_combination_of_parameters = try_combination_of_parameters)
        ...
        ### Build the final feature vector
        feature_list <- extract_feature_list_from_model_list(filepath_R = model_list, features_to_return = 0)
        ### Build the matrix with the model performances
        model_performance_matrix <- extract_performance_matrix_from_model_list(filepath_R = model_list)
        # Progress bar
        if (!is.null(progress_bar) && progress_bar == "tcltk") {
    ...
    spectral_typer_score_correlation_matrix <<- function(spectra_reference, spectra_test, filepath_reference, filepath_test, class_list_library = NULL, peaks_filtering_percentage_threshold = 5, low_intensity_percentage_threshold = 0, low_intensity_threshold_method = "element-wise", tof_mode = "linear", correlation_method = "spearman", intensity_correction_coefficient = 1, spectra_format = "fid", score_only = FALSE, allow_parallelization = FALSE, score_threshold_values = c(1.7, 2), tolerance_ppm = NULL, peak_picking_mode = "all", signals_to_take = 20, peak_picking_SNR = 3, peak_picking_algorithm = "SuperSmoother", peak_deisotoping = FALSE, peak_enveloping = FALSE, spectral_alignment_algorithm = NULL, spectral_alignment_reference = NULL) {
        ##### Load the required packages
        require(XML)
        require(MALDIquant)
        require(corrplot)
        ...
        comparison_sample_db_subfunction_correlation <- function(x) {
            ...
            colnames(number_of_signals_database_matrix) <- reference_vector
        ...
        if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
            ...
            if (Sys.info()[1] == "Linux" || Sys.info()[1] == "Darwin") {
                cpu_thread_number <- cpu_thread_number / 2
                output_list <- mclapply(reference_sample_list, FUN = function(reference_sample_list) comparison_sample_db_subfunction_correlation(reference_sample_list), mc.cores = cpu_thread_number)
            } else if (Sys.info()[1] == "Windows") {
                cpu_thread_number <- cpu_thread_number - 1
                # Make the CPU cluster for parallelisation
                ...
                output_list <- parLapply(cls, reference_sample_list, fun = function(reference_sample_list) comparison_sample_db_subfunction_correlation(reference_sample_list))
                stopCluster(cls)
            } else {
                output_list <- lapply(reference_sample_list, FUN = function(reference_sample_list) comparison_sample_db_subfunction_correlation(reference_sample_list))
            }
        } else if (is.character(allow_parallelization) && allow_parallelization == "foreach") {
            ### PARALLEL BACKEND
            require(parallel)
            ...
            output_list <- list()
            output_list <- foreach(i = 1:length(reference_sample_list), .packages = c("weights", "MALDIquant"), .export = c("peak_picking", "most_intense_signals", "align_and_filter_peaks", "align_spectra", "preprocess_spectra")) %dopar% {
                output_list[[i]] <- comparison_sample_db_subfunction_correlation(reference_sample_list[[i]])
            }
        ...
        if (score_only == TRUE) {
            ...
            for (r in 1:number_of_samples) {
                for (w in 1:reference_size) {
                    if (score[r,w] >= score_threshold_values[2]) {
                        output[r,w] <- paste0("YES\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "Corr: ", round(intensity_correlation_matrix_all[r,w], digits = 3), " , ", "p: ", pvalue_matrix_all[r,w], " , ", "sl: ", slope_matrix_all[r,w], " , ", "ns: ", matching_signals_matrix_all[r,w], ")")
                    } else if (score[r,w] < score_threshold_values[1]) {
                        output[r,w] <- paste0("NO\n","(Score: ", round(score[r,w], digits = 3), "), " ,"(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "Corr: ", round(intensity_correlation_matrix_all[r,w], digits = 3), " , ", "p: ", pvalue_matrix_all[r,w], " , ", "sl: ", slope_matrix_all[r,w], " , ", "ns: ", matching_signals_matrix_all[r,w], ")")
                    } else if (score[r,w] >= score_threshold_values[1] && score[r,w] < score_threshold_values[2]) {
                        output[r,w] <- paste0("NI\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "Corr: ", round(intensity_correlation_matrix_all[r,w], digits = 3), " , ", "p: ", pvalue_matrix_all[r,w], " , ", "sl: ", slope_matrix_all[r,w], " , ", "ns: ", matching_signals_matrix_all[r,w], ")")
    ...
    spectral_typer_score_signal_intensity <<- function(spectra_reference, spectra_test, class_list_library = NULL, reference_spectral_variability_list = NULL, test_spectral_variability_list = NULL, signal_intensity_evaluation = c("fixed percentage", "peak-wise adjusted percentage", "average coefficient of variation"), peaks_filtering_percentage_threshold = 5, low_intensity_percentage_threshold = 0, low_intensity_threshold_method = "element-wise", tof_mode = "linear", intensity_tolerance_percent_threshold = 50, spectra_format = "fid", score_only = TRUE, allow_parallelization = FALSE, score_threshold_values = c(1.7, 2), tolerance_ppm = NULL, peak_picking_mode = "all", signals_to_take = 20, peak_picking_SNR = 3, peak_picking_algorithm = "SuperSmoother", peak_deisotoping = FALSE, peak_enveloping = FALSE, spectral_alignment_algorithm = NULL, spectral_alignment_reference = NULL) {
            ...
            for (db in 1:reference_size) {
                ...
                if (signal_intensity_evaluation == "fixed percentage") {
                    ...
                    if (length(peaks_sample_x@mass) > 0 && length(peaks_reference_x@mass) > 0) {
                        ...
                        if (length(common_peaks_reference_sample) > 0) {
                            intensity_matching_sample <- 0
                            for (z in 1:length(common_peaks_reference_sample)) {
        ...
        if (score_only == TRUE) {
            ...
            for (r in 1:number_of_samples) {
                for (w in 1:reference_size) {
                    if (score[r,w] >= score_threshold_values[2]) {
                        output[r,w] <- paste0("YES\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "IntMtch: ", round(intensity_matching_matrix_all[r,w], digits = 3), " , ", "ns: ", matching_signals_matrix_all[r,w], ")")
                    } else if (score[r,w] < score_threshold_values[1]) {
                        output[r,w] <- paste0("NO\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "IntMtch: ", round(intensity_matching_matrix_all[r,w], digits = 3), " , ", "ns: ", matching_signals_matrix_all[r,w], ")")
                    } else if (score[r,w] >= score_threshold_values[1] && score[r,w] < score_threshold_values[2]) {
                        output[r,w] <- paste0("NI\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], "/", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "IntMtch: ", round(intensity_matching_matrix_all[r,w], digits = 3), " , ", "ns: ", matching_signals_matrix_all[r,w], ")")
    ...
    # Each sample gets compared with each entry in the database, separately.
    # Parallel implemented.
    spectral_typer_score_similarity_index <<- function(spectra_reference, spectra_test, filepath_reference, filepath_test, class_list_library = NULL, peaks_filtering_percentage_threshold = 5, low_intensity_percentage_threshold = 0.1, low_intensity_threshold_method = "element-wise", tof_mode = "linear", spectra_format = "fid", score_only = FALSE, allow_parallelization = FALSE, score_threshold_values = c(1.7, 2), tolerance_ppm = NULL, peak_picking_mode = "all", signals_to_take = 20, peak_picking_SNR = 3, peak_picking_algorithm = "SuperSmoother", peak_deisotoping = FALSE, peak_enveloping = FALSE, spectral_alignment_algorithm = NULL, spectral_alignment_reference = NULL) {
        ##### Load the required packages
        require(XML)
        require(MALDIquant)
        require(stats)
        ...
        if (score_only == TRUE) {
            ...
            for (r in 1:number_of_samples) {
                for (w in 1:reference_size) {
                    if (score[r,w] >= score_threshold_values[2]) {
                        output[r,w] <- paste0("YES\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "SI: ", round(similarity_index_matrix_all[r,w], digits = 3), ")")
                    } else if (score[r,w] < score_threshold_values[1]) {
                        output[r,w] <- paste0("NO\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "SI: ", round(similarity_index_matrix_all[r,w], digits = 3), ")")
                    } else if (score[r,w] >= score_threshold_values[1] && score[r,w] < score_threshold_values[2]) {
                        output[r,w] <- paste0("NI\n","(Score: ", round(score[r,w], digits = 3), "), ", "(F: ", matching_signals_matrix_all[r,w], " / ", output_list[[r]]$number_of_signals_samples, " = ", round(fit_matrix_all[r,w], digits = 3), " , ", "RF: ", matching_signals_matrix_all[r,w], " / ", number_of_signals_database_matrix_all[r,w], " = ", round(retrofit_matrix_all[r,w], digits = 3), " , ", "SI: ", round(similarity_index_matrix_all[r,w], digits = 3), ")")
    ...
    generate_custom_intensity_matrix <<- function(spectra, custom_feature_vector = NULL, tof_mode = "linear", preprocessing_parameters = list(mass_range = c(800,3000), transformation_algorithm = NULL, smoothing_algorithm = NULL, smoothing_strength = "medium", baseline_subtraction_algorithm = "SNIP", baseline_subtraction_algorithm_parameter = 100, normalization_algorithm = "TIC", normalization_mass_range = NULL, preprocess_spectra_in_packages_of = 0, spectral_alignment_algorithm = NULL), peak_picking_algorithm = "SuperSmoother", peak_picking_SNR = 5, peak_filtering_frequency_threshold_percent = 5, low_intensity_peak_removal_threshold_percent = 0, low_intensity_peak_removal_threshold_method = "element-wise", allow_parallelization = FALSE, deisotope_peaklist = FALSE, tolerance_ppm = NULL) {
        ...
        if (!is.null(custom_feature_vector)) {
            ...
            if (feature_compatibility == TRUE) {
                ...
                if (isMassPeaksList(peaks)) {
                    if ((is.logical(allow_parallelization) && allow_parallelization == TRUE) || (is.character(allow_parallelization) && allow_parallelization == "lapply")) {
                        ...
                        # Run the function
                        peaks <- foreach(i = 1:length(peaks), .packages = "MALDIquant") %dopar% {
                            peaks[[i]] <- peak_alignment_subfunction(peaks = peaks[[i]], reference_masses = custom_feature_vector, tolerance_ppm = tolerance_ppm)
                        }
                        names(peaks) <- list_names
                    } else {
                        peaks <- lapply(peaks, FUN = function(peaks) peak_alignment_subfunction(peaks = peaks, reference_masses = custom_feature_vector, tolerance_ppm = tolerance_ppm))
            ...
            peaks <- align_and_filter_peaks(peaks, peak_picking_algorithm = peak_picking_algorithm, tof_mode = tof_mode, peak_filtering_frequency_threshold_percent = peak_filtering_frequency_threshold_percent, low_intensity_peak_removal_threshold_percent = low_intensity_peak_removal_threshold_percent, low_intensity_peak_removal_threshold_method = low_intensity_peak_removal_threshold_method, reference_peaklist = NULL, spectra = spectra, alignment_iterations = 5, allow_parallelization = allow_parallelization, tolerance_ppm = tolerance_ppm)
    ...
    partition_spectral_dataset <<- function(spectra, partitioning_method = c("space","random", "hca"), number_of_partitions = 3, seed = NULL, tof_mode = "reflectron") {
        ...
        if (isMassSpectrumList(spectra)) {
            ...
            if (partitioning_method == "space") {
                ...
                peaks <- align_and_filter_peaks(peaks, tof_mode = tof_mode, peak_filtering_frequency_threshold_percent = 5, tolerance_ppm = tolerance_ppm)
                ### Generate the peaklist matrix
                peaklist <- intensityMatrix(peaks, spectra)
                ### Compute the distance matrix
                distance_matrix <- dist(peaklist, method = "euclidean")
    ...
    extract_feature_list_from_model_list <<- function(filepath_R, features_to_return = 20) {
        ...
        model_features <- as.matrix(cbind(model_features))
        colnames(model_features) <- "Features"
        ### Return
    ...
    extract_common_features_from_model_list <<- function(filepath_R, features_to_return = 20) {
        if (!is.list(filepath_R)) {
            ### LOAD THE R WORKSPACE WITH THE MODEL LIST
            # Create a temporary environment
            temporary_environment <- new.env()
    ...
    extract_performance_matrix_from_model_list <<- function(filepath_R) {
        if (!is.list(filepath_R)) {
            ### LOAD THE R WORKSPACE WITH THE MODEL LIST
            # Create a temporary environment
            temporary_environment <- new.env()
...
spectral_typer <- function() {
    ...
    database_dump_function <- function() {
        ...
        if (!is.null(spectra_reference)) {
            ...
            if (peak_picking_mode == "all") {
                peaks_reference <- peak_picking(spectra = spectra_reference, peak_picking_algorithm = peak_picking_algorithm, tof_mode = tof_mode, SNR = SNR, allow_parallelization = allow_parallelization, deisotope_peaklist = peak_deisotoping, envelope_peaklist = peak_enveloping)
            } else if (peak_picking_mode == "most intense") {
                peaks_reference <- most_intense_signals(spectra = spectra_reference, signals_to_take = signals_to_take, tof_mode = tof_mode, peak_picking_algorithm = peak_picking_algorithm, allow_parallelization = allow_parallelization, deisotope_peaklist = peak_deisotoping, envelope_peaklist = peak_enveloping)
            }
    ...
    run_spectral_typer_function <- function() {
        ...
        if (!is.null(spectra_reference) && !is.null(spectra_test)) {
            ...
            if ("similarity index" %in% similarity_criteria) {
                ...
                try({
                    score_si_matrix <- spectral_typer_score_similarity_index(spectra_reference, spectra_test, filepath_reference, filepath_test, class_list_library = reference_folder_list, peaks_filtering_percentage_threshold = peaks_filtering_threshold_percent, low_intensity_percentage_threshold = low_intensity_peak_removal_percentage_threshold, low_intensity_threshold_method = low_intensity_peak_removal_threshold_method, tof_mode = tof_mode, spectra_format = spectra_format, score_only = score_only, allow_parallelization = allow_parallelization, score_threshold_values = score_threshold_values, tolerance_ppm = tolerance_ppm, peak_picking_mode = peak_picking_mode, signals_to_take = signals_to_take, peak_picking_SNR = SNR, peak_picking_algorithm = peak_picking_algorithm, peak_deisotoping = peak_deisotoping, peak_enveloping = peak_enveloping, spectral_alignment_algorithm = spectral_alignment_algorithm, spectral_alignment_reference = spectral_alignment_reference)
            ...
            # Parameters vector
            parameters_vector <- c(file_type_export, filepath_reference, filepath_test, mass_range_value, tof_mode_value, tolerance_ppm_value, spectra_format_value, preprocess_spectra_in_packages_of_value, peak_picking_mode, peak_picking_algorithm_value, signals_to_take_value, intensity_tolerance_percent_value, similarity_criteria_value, intensity_correction_coefficient_value, SNR_value, peaks_filtering_threshold_percent_value, low_intensity_peak_removal_percentage_threshold_value, average_replicates_in_reference_value, average_replicates_in_test_value, score_only_value, score_threshold_values_value)
            names(parameters_vector) <- c("File type", "Reference folder", "Samples folder", "Mass range", "TOF mode", "Tolerance (in ppm)", "Spectra format", "Preprocess spectra in packages of", "Peak picking mode", "Peak picking algorithm", "Most intense signals taken", "Intensity tolerance percent", "Similarity criteria", "Intensity correction coefficient", "Signal-to-noise ratio", "Peaks filtering threshold percentage", "Low intensity peaks removal threshold percent", "Average replicates in the reference", "Average replicates in the samples", "Score only", "Score threshold values")
            ...
            if (file_type_export == "csv") {
                ...
                # Intensity
                if (!is.null(score_intensity_matrix)) {
                    write.csv(score_intensity_matrix, file = paste0("INTENSITY - ", filename))
                }
                # HCA
                if (!is.null(score_hca)) {
                    # Dump the hca plot
                ...
                # Correlation
                if (!is.null(score_correlation_matrix)) {
                    write.csv(score_correlation_matrix, file = paste0("CORRELATION - ", filename))
                }
                # Similarity index
                if (!is.null(score_si_matrix)) {
                    write.csv(score_si_matrix, file = paste0("SI - ", filename))
                }
                # CV matrix
                ...
                if (!is.null(score_intensity_matrix)) {
                    ...
                    wb <- loadWorkbook(filename = paste0("INTENSITY - ", filename), create = TRUE)
                    # Create the sheet
                ...
                if (!is.null(score_correlation_matrix)) {
                    ...
                    # Load the workbook (create if there is not already)
                    wb <- loadWorkbook(filename = paste0("CORRELATION - ", filename), create = TRUE)
                    # Create the sheet
                ...
                if (!is.null(score_si_matrix)) {
                    ...
                    # Load the workbook (create if there is not already)
                    wb <- loadWorkbook(filename = paste0("SI - ", filename), create = TRUE)
                    # Create the sheet
