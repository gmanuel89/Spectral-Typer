#################### FUNCTIONS - MASS SPECTROMETRY 2017.05.30 ##################
...
extract_bayesian_probabilities <- function(training_set, model_object, discriminant_attribute = "Class", non_features = c("Sample", "Class"), type_of_analysis = "cross validation", test_set = NULL) {
    ...
    if (type_of_analysis == "cross validation" || type_of_analysis == "cv" || type_of_analysis == "CV") {
        ...
        for (cl in 1:length(class_list)) {
            ...
            model_cv_performance_parameter_list[[positive_class_cv]][["likelihood"]] <- model_cv_performance_parameter_vector
        ...
        for (cl in 1:length(class_list)) {
            ...
            model_external_performance_parameter_list[[positive_class_cv]][["likelihood"]] <- model_external_performance_parameter_vector
            # Store the prior probability in the list, named according to the positive class of the CV (prior)
            model_cv_performance_parameter_list[[positive_class_cv]][["prior"]] <- nrow(training_set[training_set[, discriminant_attribute] == positive_class_cv, ]) / nrow(training_set)
        }
        ### Return
...
select_database_function <- function() {
    ...
    .GlobalEnv$filepath_database_value <- filepath_database_value
}
...
select_samples_function <-function() {
    ...
    .GlobalEnv$filepath_test_value <- filepath_test_value
}
...
similarity_criteria_choice <- function() {
    ...
    if ("signal intensity" %in% similarity_criteria) {
        ...
        .GlobalEnv$similarity_criteria_value_signal_intensity <- similarity_criteria_value_signal_intensity
    }
    # Displaying value
    ...
    for (s in 1:length(similarity_criteria)) {
        if (is.null(similarity_criteria_value)) {
            ...
            } else if (similarity_criteria[s] == "signal intenity") {
...
allow_parallelization_choice <- function() {
    ...
    # Catch the value from the menu
    allow_parallelization <- select.list(c("YES","NO"), title="Allow parallel computing", preselect = "NO")
...
import_spectra_function <- function() {
    if (!is.null(filepath_database) && !is.null(filepath_test)) {
        ...
        # Progress bar
        setTkProgressBar(import_progress_bar, value = 0.70, title = "Estimating spectral variability...", label = "70 %")
...
run_spectral_typer_function <- function() {
    ...
    if (!is.null(spectra_database) && !is.null(spectra_test) && !is.null(peaks_database) && !is.null(peaks_test)) {
        ...
        if (!is.null(database_cv_matrix)) {
            .GlobalEnv$database_cv_matrix <- database_cv_matrix
        }
