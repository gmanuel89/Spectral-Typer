...
functions_mass_spectrometry <- function() {
  ...
  ################## FUNCTIONS - MASS SPECTROMETRY 2018.04.03 ################
    ...
    if (method == "getURL") {
      try({
        ...
        if ("RCurl" %in% installed.packages()[,1]) {
          ...
          install.packages("RCurl", repos = "http://cran.mirror.garr.it/mirrors/CRAN/", quiet = TRUE, verbose = FALSE, lib = Sys.getenv("R_LIBS_USER"), destdir = Sys.getenv("R_LIBS_USER"))
          library(RCurl)
        }
      }, silent = TRUE)
      there_is_internet <- FALSE
...
#  ##################################################### INSTALL REQUIRED PACKAGES (FROM SPECIFIED PACKAGE SET)
#  # This function installs and loads the selected packages
# install_and_load_required_packages_from_package_set <<- function(required_packages, online_repository = NULL, local_repository = Sys.getenv("R_LIBS_USER"), update_packages = FALSE, print_messages = FALSE) {
#   # If the local repository does not exist, or the online repository is not specified, install from internet
#   if (online_repository == NULL || online_repository == "" || !dir.exists(local_repository)) {
#       install_and_load_required_packages(required_packages = required_packages, repository = "http://cran.mirror.garr.it/mirrors/CRAN/", update_packages = update_packages, print_messages = print_messages)
#   }
#   
#   # Try locally
...
#       install.packages(missing_packages, repos = NULL, quiet = TRUE, verbose = FALSE, lib = Sys.getenv("R_LIBS_USER"), destdir = Sys.getenv("R_LIBS_USER"))
#   }
...
#  }
    ...
    if (update_packages == TRUE) {
      if (there_is_internet == TRUE) {
        ...
        if (repository != "" || !is.null(repository)) {
          ...
          .libPaths(Sys.getenv("R_LIBS_USER"))
          update.packages(repos = repository, ask = FALSE, checkBuilt = TRUE, quiet = TRUE, verbose = FALSE, lib.loc = Sys.getenv("R_LIBS_USER"), destdir = Sys.getenv("R_LIBS_USER"))
        } else {
          # Check for the personal local library presence before installing (~/R/packages/), then install in the local library
          if (!dir.exists(Sys.getenv("R_LIBS_USER"))) {
            dir.create(Sys.getenv("R_LIBS_USER"))
          ...
          .libPaths(Sys.getenv("R_LIBS_USER"))
          update.packages(ask = FALSE, checkBuilt = TRUE, quiet = TRUE, verbose = FALSE, lib.loc = Sys.getenv("R_LIBS_USER"), destdir = Sys.getenv("R_LIBS_USER"))
        }
    ...
    if (length(missing_packages) > 0) {
      ...
      if (there_is_internet == TRUE) {
        ...
        if (repository != "" || !is.null(repository)) {
          ...
          .libPaths(Sys.getenv("R_LIBS_USER"))
          install.packages(missing_packages, repos = repository, quiet = TRUE, verbose = FALSE, lib = Sys.getenv("R_LIBS_USER"), destdir = Sys.getenv("R_LIBS_USER"))
        } else {
          ...
          .libPaths(Sys.getenv("R_LIBS_USER"))
          install.packages(missing_packages, quiet = TRUE, verbose = FALSE, lib = Sys.getenv("R_LIBS_USER"), destdir = Sys.getenv("R_LIBS_USER"))
        }
        if (print_messages == TRUE) {
      ...
      bayesian_ensemble_predictions_subfunction <- function(x) {
        ...
        for (m in 1:length(model_performance_parameter_list)) {
          ...
          for (cl in 1:length(class_list)) {
            ...
            if (bayesian_prob == 0) {
              bayesian_prob <- as.numeric(alpha)
            } else if (bayesian_prob == 1) {
              bayesian_prob <- 1 - as.numeric(alpha)
            }
  ...
  single_model_classification_of_spectra <<- function(spectra, model_x, model_name = "model", preprocessing_parameters = NULL, peak_picking_algorithm = "SuperSmoother", deisotope_peaklist = FALSE, envelope_peaklist = FALSE, peak_picking_SNR = 5, peak_filtering_frequency_threshold_percent = 5, low_intensity_peak_removal_threshold_percent = 1, low_intensity_peak_removal_threshold_method = "element-wise", tof_mode = "linear", allow_parallelization = FALSE, pixel_grouping = c("single", "hca", "moving window average", "graph"), number_of_hca_nodes = 5, moving_window_size = 5, seed = 12345, correlation_method_for_adjacency_matrix = "pearson", correlation_threshold_for_adjacency_matrix = 0.95, pvalue_threshold_for_adjacency_matrix = 0.05, max_GA_generations = 10, iterations_with_no_change = 5, classification_mode_graph = c("average spectra", "single spectra clique"), plot_figures = TRUE, plot_graphs = TRUE, plot_legends = c("sample name", "legend", "plot name"), features_to_use_for_graph = c("all", "model"), tolerance_ppm = NULL) {
    ...
    if (isMassSpectrumList(spectra)) {
      ...
      ### Generate the intensity matrix with the features from the model
      final_sample_matrix <- generate_custom_intensity_matrix(spectra, custom_feature_vector = features_model, tof_mode = tof_mode, preprocessing_parameters = NULL, peak_picking_algorithm = peak_picking_algorithm, peak_picking_SNR = 3, peak_filtering_frequency_threshold_percent = NULL, low_intensity_peak_removal_threshold_percent = 0, low_intensity_peak_removal_threshold_method = "element-wise", allow_parallelization = allow_parallelization, deisotope_peaklist = deisotope_peaklist, envelope_peaklist = envelope_peaklist, tolerance_ppm = tolerance_ppm)
    ...
    for (p in 1:length(filepath_test_imzml)) {
      ...
      if ("profile" %in% classification_mode) {
        if (length(list_of_models) > 2 && !is.null(final_result_matrix_profile_patient) && classes_are_the_same_for_each_model == TRUE && outcomes_are_the_same_for_each_model == TRUE) {
          ########## Ensemble results
          classification_ensemble_matrix_profile <- ensemble_vote_classification(classification_matrix = final_result_matrix_profile_patient, class_list = model_list[[1]]$class_list, weighted_decision_method = decision_method_ensemble, classification_probabilities_list = predicted_classes_probs_list, performance_parameter_list = model_performance_parameter_list, type_of_validation_for_performance_estimation = "cv", alpha = alpha)
          # Store the ensemble classification matrix in the final output list
  ...
  graph_MSI_segmentation <<- function(filepath_imzml, preprocessing_parameters = list(mass_range = c(800,3000), transformation_algorithm = NULL, smoothing_algorithm = NULL, smoothing_strength = "medium", baseline_subtraction_algorithm = "SNIP", baseline_subtraction_algorithm_parameter = 200, normalization_algorithm = "TIC", normalization_mass_range = NULL, process_spectra_in_packages_of = 0, spectral_alignment_algorithm = NULL), tolerance_ppm = NULL, allow_parallelization = FALSE, peak_picking_algorithm = "SuperSmoother", deisotope_peaklist = FALSE, envelope_peaklist = FALSE, SNR = 5, tof_mode = "reflectron", peak_filtering_frequency_threshold_percent = 5, low_intensity_peak_removal_threshold_percent = 0, low_intensity_peak_removal_threshold_method = "element-wise", custom_feature_vector = NULL, correlation_method_for_adjacency_matrix = "pearson", correlation_threshold_for_adjacency_matrix = 0.90, pvalue_threshold_for_adjacency_matrix = 0.05, number_of_high_degree_vertices_for_subgraph = 0, vertices_not_in_induced_subgraph = c("independent", "reassigned"), max_GA_generations = 10, iterations_with_no_change = 5, plot_figures = TRUE, plot_graphs = TRUE, plot_legends = c("sample name", "legend", "plot name"), seed = 12345, spectra_format = "imzML") {
    # Install and load the required packages
    require(XML)
    require(MALDIquantForeign)
    require(MALDIquant)
...
spectral_typer <- function() {
  ...
  ############## INSTALL AND LOAD THE REQUIRED PACKAGES
  install_and_load_required_packages(c("tcltk", "ggplot2", "ggdendro", "XML", "MALDIquant", "MALDIquantForeign", "weights", "stats", "foreach", "parallel", "corrplot", "weights"), repository = "http://cran.mirror.garr.it/mirrors/CRAN/", update_packages = FALSE, print_messages = TRUE)
  if (Sys.info()[1] == "Windows") {
